def deploy(id) {
    ...
    if (id.equals("staging") || id.equals("production") || id.equals("qa")) {
 	...
 	sh "[ -z \$(docker images -q 1221-appdeploy-staging) ] || docker rm \$(docker stop \$(docker ps -a -q --filter ancestor=1221-appdeploy-staging)) || \
            docker rmi 1221-appdeploy-staging"
        sh "[ -z \$(docker images -q 1221-appdeploy-production) ] || docker rm \$(docker stop \$(docker ps -a -q --filter ancestor=1221-appdeploy-production)) 		 || docker rmi 1221-appdeploy-production"
